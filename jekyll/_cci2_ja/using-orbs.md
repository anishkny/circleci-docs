...
This document provides a basic example of importing an [orb]({{ site.baseurl }}/ja/2.0/orb-intro/) and elements of orbs followed by a conceptual overview of orbs design. 
...
To use an existing orb in your 2.1 [`.circleci/config.yml`]({{ site.baseurl }}/ja/2.0/configuration-reference/#orbs-requires-version-21) file, invoke it with the `orbs` key. The following example invokes the [`hello-build` orb](https://circleci.com/orbs/registry/orb/circleci/hello-build) in the `circleci` namespace.
