...
To customize the image, create a Dockerfile that builds `FROM` the `circleci/android` image. See [Using Custom-Built Docker Images]({{ site.baseurl }}/ja/2.0/custom-images/) for instructions.
...
Follow the instructions in the [Authorizing the Google Cloud SDK]({{ site.baseurl }}/ja/2.0/google-auth/) document.

    **Note:**
    Instead of `google/cloud-sdk`,
    consider using an [Android convenience image]({{ site.baseurl }}/ja/2.0/circleci-images/#android),
...
See the [Deploy]({{ site.baseurl }}/ja/2.0/deployment-integrations/) document for examples of deploy target configurations.
