...
This document describes how to version your [.circleci/config.yml]({{ site.baseurl }}/ja/2.0/configuration-reference/) file and get started with reusable orbs, commands, jobs, and executors.
...
1. Add your project on the Add Projects page if it is a new project. For an existing Project, go to the Project Settings and enable [Build Processing]({{ site.baseurl }}/ja/2.0/build-processing/) with the radio button.

2. (Optional) Install the CircleCI-Public CLI by following the [Using the CircleCI CLI]({{ site.baseurl }}/ja/2.0/local-cli/) documentation. The `circleci config process` command is helpful for checking reusable config.
