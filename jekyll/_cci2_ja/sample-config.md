...
This document provides three sample [`.circleci/config.yml`]({{ site.baseurl }}/ja/2.0/configuration-reference/) files, as follows:

* TOC
{:toc}

The CircleCI 2.0 configuration introduces a new key for `version: 2`. This new key enables you to try 2.0 while continuing to build on 1.0. That is, you can still use 1.0 on some projects while using 2.0 on others. New keys for `jobs` and `steps` and `workflows` enable greater control and status on each phase of a run to report more frequent feedback. See [Jobs and Steps]({{ site.baseurl }}/ja/2.0/jobs-steps/) and [Workflows]({{ site.baseurl }}/ja/2.0/workflows/) for more details.
...
This example shows a sequential workflow with the `test` job configured to run only on the master branch. Refer to the [Workflows]({{ site.baseurl }}/ja/2.0/workflows) document for complete details about orchestrating job runs with parallel, sequential, and manual approval workflows.
...
See the [Example Public Repos]({{ site.baseurl }}/ja/2.0/example-configs/) document for a list of public projects that use CircleCI.
