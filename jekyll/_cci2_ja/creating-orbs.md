...
Orbs are reusable packages of CircleCI configuration that you may share across projects, enabling you to create encapsulated, parameterized commands, jobs, and executors that can be [used across multiple projects]({{ site.baseurl }}/ja/2.0/using-orbs/). 

[Orbs]({{ site.baseurl }}/ja/2.0/orb-intro/) are made available for use in a configuration through the `orbs` key in the top level of your 2.1 [.circleci/config.yml]({{ site.baseurl }}/ja/2.0/configuration-reference/) file.
...
**Note:** Running `circleci setup`, providing a personal API token, and enabling [Build Processing]({{ site.baseurl }}/ja/2.0/build-processing/) for your project are prerequisites for creating orbs.

Give it a try! If you have any trouble, check out the additional details and tips below or the [Orbs FAQ]({{ site.baseurl }}/ja/2.0/orbs-faq/).
...
Refer to [Reusing Config]({{ site.baseurl }}/ja/2.0/reusing-config/) for details and examples of commands, executors and parameters in orbs.
...
In the CircleCI app Settings page for your project, [Build Processing]({{ site.baseurl }}/ja/2.0/build-processing/) must be enabled (default is to be ON for all new projects). The org Admin must also opt-in to use of uncertified orbs in your organization under the Settings tab on the Security page of the CircleCI app.
