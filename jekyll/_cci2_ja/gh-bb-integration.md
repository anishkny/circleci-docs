...
Refer to CircleCI documentation of [Workflows filters]({{ site.baseurl }}/ja/2.0/workflows/#using-contexts-and-filtering-in-your-workflows) for how to build tag pushes. 

### Add a .circleci/config.yml File
{:.no_toc}

After you create and commit a [`.circleci/config.yml`]({{ site.baseurl }}/ja/2.0/configuration-reference/) file to your GitHub or Bitbucket repository CircleCI immediately checks your code out and runs your first job along with any configured tests. For example, if you are working on a Rails project using Postgres specifications and features you might configure the following job run step:
...
GitHub repositories. Refer to the [adding ssh keys]({{ site.baseurl }}/ja/2.0/add-ssh-key) document to learn more.
