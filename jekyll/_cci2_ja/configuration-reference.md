...
----|-----------|------|------------
version | Y | String | `2`, `2.0`, or `2.1` See the [Reusing Config]({{ site.baseurl }}/ja/2.0/reusing-config/) doc for an overview of new 2.1 keys available to simplify your `.circleci/config.yml` file, reuse, and parameterized jobs.
...
Key | Required | Type | Description
----|-----------|------|------------
orbs | N | Map | A map of user-selected names to either: orb references (strings) or orb definitions (maps). Orb definitions must be the orb-relevant subset of 2.1 config. See the [Creating Orbs]({{ site.baseurl }}/ja/2.0/creating-orbs/) documentation for details.
...
A command definition defines a sequence of steps as a map to be executed in a job, enabling you to [reuse a single command definition]({{ site.baseurl }}/ja/2.0/reusing-config/) across multiple jobs.
...
See the Using Parameters in Executors section of the [Reusing Config]({{ site.baseurl }}/ja/2.0/reusing-config/) document for examples of parameterized executors.

## **`jobs`**

A run is comprised of one or more named jobs. Jobs are specified in the
`jobs` map, see [Sample 2.0 config.yml]({{ site.baseurl }}/ja/2.0/sample-config/) for two examples of a `job` map. The name of the job is the key in the map, and the value is a map
describing the job.

If you are using [Workflows]({{ site.baseurl }}/ja/2.0/workflows/), jobs must have a name that is unique within the `.circleci/config.yml` file.
...
If `parallelism` is set to N > 1, then N independent executors will be set up and each will run the steps of that job in parallel. Certain parallelism-aware steps can opt out of the parallelism and only run on a single executor (for example [`deploy` step](#deploy)). Learn more about [parallel jobs]({{ site.baseurl }}/ja/2.0/parallelism-faster-jobs/).
...
An "executor" is roughly "a place where steps occur". CircleCI 2.0 can build the necessary environment by launching as many docker containers as needed at once, or it can use a full virtual machine. Learn more about [different executors]({{ site.baseurl }}/ja/2.0/executor-types/).
...
For [primary container]({{ site.baseurl }}/ja/2.0/glossary/#primary-container) (listed first in the list) if no `command` is specified then `command` and image entrypoint will be ignored, to avoid errors caused by the entrypoint executable consuming significant resources or exiting prematurely. At this time all `steps` run in the primary container only.
...
You can specify image versions using tags or digest. You can use any public images from any public Docker registry (defaults to Docker Hub). Learn more about [specifying images]({{ site.baseurl }}/ja/2.0/executor-types).
...
It is possible to reuse [declared commands]({{ site.baseurl }}/ja/2.0/reusing-config/) in a job when using version 2.1. The following example invokes the `sayhello` command. 
...
The [machine executor]({{ site.baseurl }}/ja/2.0/executor-types) is configured by using the `machine` key, which takes a map:

Key | Required | Type | Description
----|-----------|------|------------
enabled | N | Boolean | This must be true in order to enable the `machine` executor.  Is required if no other value is specified
image | N | String | The image to use (default: `circleci/classic:latest`). **Note:** This key is **not** supported on the installable CircleCI. For information about customizing `machine` executor images on CircleCI installed on your servers, see our [VM Service documentation]({{ site.baseurl }}/ja/2.0/vm-service).
docker_layer_caching | N | Boolean | Set to `true` to enable [Docker Layer Caching]({{ site.baseurl }}/ja/2.0/docker-layer-caching). **Note:** You must open a support ticket to have a CircleCI Sales representative contact you about enabling this feature on your account for an additional fee.
{: class="table table-striped"}
...
The machine executor supports [Docker Layer Caching]({{ site.baseurl }}/ja/2.0/docker-layer-caching) which is useful when you are building Docker images during your job or Workflow.
...
Key | Required | Type | Description
----|-----------|------|------------
xcode | Y | String | The version of Xcode that is installed on the virtual machine, see the [Supported Xcode Versions section of the Testing iOS]({{ site.baseurl }}/ja/2.0/testing-ios/#supported-xcode-versions) document for the complete list.
{: class="table table-striped"}
...
see the [Using Shell Scripts]({{ site.baseurl }}/ja/2.0/using-shell-scripts/) document.
...
Creates a remote Docker environment configured to execute Docker commands. See [Running Docker Commands]({{ site.baseurl }}/ja/2.0/building-docker-images/) for details.

Key | Required | Type | Description
----|-----------|------|------------
docker_layer_caching | N | boolean | set this to `true` to enable [Docker Layer Caching]({{ site.baseurl }}/ja/2.0/docker-layer-caching/) in the Remote Docker Environment (default: `false`)
{: class="table table-striped"}
...
- *`setup_remote_docker` is not compatible with the `machine` executor. See [Docker Layer Caching in Machine Executor]({{ site.baseurl }}/ja/2.0/docker-layer-caching/#machine-executor) for information on how to enable DLC with the `machine` executor.*

##### **`save_cache`**

Generates and stores a cache of a file or directory of files such as dependencies or source code in our object storage. Later jobs can [restore this cache](#restore_cache). Learn more in [the caching documentation]({{ site.baseurl }}/ja/2.0/caching/).
...
Restores a previously saved cache based on a `key`. Cache needs to have been saved first for this key using [`save_cache` step](#save_cache). Learn more in [the caching documentation]({{ site.baseurl }}/ja/2.0/caching/).
...
Step to store artifacts (for example logs, binaries, etc) to be available in the web app or through the API. See the   [Uploading Artifacts]({{ site.baseurl }}/ja/2.0/artifacts/) document for more information.
...
###### **`context`**
Jobs may be configured to use global environment variables set for an organization, see the [Contexts]({{ site.baseurl }}/ja/2.0/contexts) document for adding a context in the application settings.
...
For more information,
see the [Executing Workflows For a Git Tag]({{ site.baseurl }}/ja/2.0/workflows/#executing-workflows-for-a-git-tag) section of the Workflows document.
...
Refer to the [Orchestrating Workflows]({{ site.baseurl }}/ja/2.0/workflows) document for more examples and conceptual information.
