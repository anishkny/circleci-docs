...
installed in a Docker image. If you’re in a rush, just copy the sample configuration below into a [`.circleci/config.yml`]({{ site.baseurl }}/ja/2.0/configuration-reference/) in your project’s root directory and start building.
...
Directly beneath `working_directory`, we’ll specify [primary container]({{ site.baseurl }}/ja/2.0/glossary/#primary-container) images for this job under `docker`.
...
Our project uses `make` for building and testing (you can see `Makefile` [here](https://github.com/CircleCI-Public/circleci-demo-go/blob/master/Makefile)), so we can just run `make test`. In order to collect test results and upload them later (read more about test results in the [Project Tutorial]({{ site.baseurl }}/ja/2.0/project-walkthrough/)) we're using `go-junit-report`:
...
Refer to the [Caching Dependencies]({{ site.baseurl }}/ja/2.0/caching/) document for more caching strategies.
